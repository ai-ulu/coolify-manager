...
- Natural-language operation routing with approval-gated mutating actions.
...
- `agents/orchestrator_agent.py`: intent router and role-based sub-agent coordinator.
...
### Orchestrator Model
- Parent agent: `OrchestratorAgent`.
- Sub-agents (internal roles):
  - Monitoring (status/report generation)
  - Deploy (app lifecycle actions)
  - Backup (backup/restore requests)
  - Security (safe config report)
  - Reporter (approval + operator messages)
- Approval-gated actions:
  - `deploy_app`, `restart_app`, `start_app`, `stop_app`
- Telegram approval flow:
  - Request action in natural language
  - `/pending` to view queued approvals
...
- Mutating natural-language actions are never executed immediately; approval token is required.
...
- Natural-language status/config queries produce structured responses.
